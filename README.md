...
data class ParsingRule(
    val id: String,              // 规则ID
    val companyName: String,     // 快递公司名称
    val parcelCodePattern: String, // 取件码正则
    val addressPattern: String?,   // 地址正则（可选）
    val isEnabled: Boolean = true  // 是否启用
)
```

### 4. 用户界面
- **主界面**：底部导航栏（首页/取件历史/设置），iOS液态风格
- **快捷操作**：左侧显示未取件数量，一键标记全部已取件
- **短信详情**：点击卡片查看原始短信完整信息
- **左滑操作**：左滑标记为已取件（无确认弹窗，全高度绿色背景揭示）
- **剪贴板识别**：打开应用时自动检测剪贴板中的取件码
- **扫描功能**：顶部"扫描"按钮，按用户自定义时间范围扫描短信
- **规则管理**：按快递公司分组，支持一键启用/禁用
- **设置页面**：全屏页面（非弹窗），分组显示扫描设置、快捷操作等
- **权限提示**：友好的权限申请界面
...
### v0.2 (2026-04-17)
- **底部导航栏**：新增首页/取件历史/设置三个标签页
- **快捷操作**：左侧显示未取件数量，支持一键全部标记已取件
- **交互优化**：点击卡片查看原始短信，左滑标记已取件（无需确认）
- **剪贴板识别**：启动应用时自动检测剪贴板中的取件码
- **时间范围扫描**：顶部"扫描"按钮，按自定义时间范围扫描短信
- **规则管理**：每个快递公司一个预设规则，支持一键启用/禁用
- **设置页面**：从弹窗改为全屏页面
- **应用图标**：黄色快递包裹主题图标
- **UI美化**：整体iOS液态风格
